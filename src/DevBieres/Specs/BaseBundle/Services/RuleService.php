...
class RuleService extends BaseService implements IEntityFormService
{
     ...
     * FullName
	 */
    protected function getFullEntityName() { return 'DevBieres\Specs\BaseBundle\Entity\Rule'; }

    /**
	 * Get the document form
	 */
	public function getForm() { return new \DevBieres\Specs\BaseBundle\Form\RuleType(); }
	...
	/**
	 * Get a new code for a rule
	 * @param Document $document 
	 */
	public function getNewCode($document) {
			// Repo
			$max = $this->getRepo()->findMaxRuleCode($document->getId());
			// Depeding
			if(null === $max) { $max = 0; }
			else {
			   $max = substr($max, 1);
			...
			$c = count($n);
			// Build the code
			return sprintf("R%s%s", str_repeat("0", 5 - $c), $n);
